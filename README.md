...
selectrs aims to be a drop-in replacement for selectr: the exported functions and their arguments are the same, and queries match the same nodes. The generated XPath expressions are equivalent, though not always byte-identical (for example, the Rust core parenthesises predicates more eagerly). The main difference is under the hood — selector parsing and translation happen in Rust rather than R, which makes translation substantially faster.

Selectors that use constructs selectrs does not support raise an error naming the construct, mirroring selectr's behaviour.
