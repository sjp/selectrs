# Document-querying blocks ported from selectr's tests/testthat/test-where.R
...
test_that(":where() works correctly with XML documents", {
    ...
    expect_equal(get_ids("*:where(div, p)"), c("d1", "d2", "p1", "p2"))

    # Elements with class content (any element type)
    expect_equal(get_ids("*:where(.content)"), c("d1", "p1", "s1"))

    # Div elements that are either content or sidebar
    expect_equal(get_ids("div:where(.content, .sidebar)"), c("d1", "d2"))
    ...
    expect_equal(get_ids("*:where(p.highlight)"), "p2")

    # :where() matches nothing if conditions don't align
    expect_equal(length(querySelectorAll(doc, "div:where(p)")), 0)
})
