...
# adjacent-sibling, and :lang() against XML-package documents. Translation
...
test_that(":has() matches elements with matching descendants/siblings", {
    ...
    get_ids <- function(css) sapply(querySelectorAll(doc, css), xmlGetAttr, "id")
...
test_that("the adjacent sibling combinator matches only an immediately following sibling", {
    ...
    doc <- xmlRoot(xmlParse('<r><a id="a1">A</a><b id="b1">B</b><c id="c1">C</c></r>'))
    get_ids <- function(css) sapply(querySelectorAll(doc, css), xmlGetAttr, "id")
